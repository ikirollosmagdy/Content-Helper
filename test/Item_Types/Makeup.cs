        ...
        public void Organize()
        {
            ...
            for (row = 0; row < Form1.Sheet.RowCount; row++)
            {
                for (col = 0; col < Form1.Sheet.ColumnCount; col++)
                {
                    ...
                    try
                    {
                        if (Form1.Sheet.Columns[col].HeaderText.ToLower().Contains("model") || (Form1.Sheet.Columns[col].HeaderText.ToLower().Contains("code")))
                        {
                            Form1.OrganizedSheet[Model, row].Value = Form1.Sheet[col, row].Value;
                            break;
                        }
                        else
                        {
